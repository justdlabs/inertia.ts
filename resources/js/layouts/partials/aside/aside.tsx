...
export function AsideLink({ className, active, ...props }: AsideLinkProps) {
    return (
        <li className="-mx-1">
            <Link
                className={cn(
                    active ? 'text-fg font-semibold' : 'text-muted-fg',
                    'flex items-center [&>svg]:size-4 [&>svg]:stroke-[1.25] [&>svg]:mr-2 [&>svg]:-ml-1 hover:bg-accent/50 tracking-tight text-sm hover:text-fg px-4 py-2 rounded-md'
                )}
                {...props}
            />
        </li>
...
export function AsideLabel({ children, className }: PropsWithChildren<{ className?: string }>) {
    return (
        <li className="-mx-4">
            <span
                className={cn(
                    'flex items-center text-muted-fg [&>svg]:w-4 [&>svg]:stroke-[1.25] [&>svg]:h-4 [&>svg]:mr-3 tracking-tight text-sm px-4 py-2 rounded-md',
                    className
                )}
            >
                {children}
